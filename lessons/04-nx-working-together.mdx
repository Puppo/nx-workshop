...
In this section, we shared the from backend to frontend to prevent integration
bugs.

The first step is creating a new agnostic framework library that contains the
share interfaces. For to do this, we need to add another nx schematics
"nrwl/web"
...
add new file to proxy http request in dev module
...
and add this configuration in the workspace.json
...
now in two different terminal run
...
Navigate to the [angular app](http://localhost:4200) and now you show the
angular app fetching data from the api app
